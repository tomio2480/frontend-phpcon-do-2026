...
Gemini レビューで「フックにドメイン知識を混入すべきでない」という指摘を受けた．
汎用の `toggleCodes(codes: string[])` に変更した．

`SAPPORO_CODES` は `src/constants.ts` に移動した．
`App.tsx` 側で `useCallback` を使って組み立てる設計にした．

### regionActions パターンで CheckboxList を汎用化する

当初 `CheckboxList` はドメイン定数をコンポーネント内に直接持っていた．
`ISHIKARI_REGION`・`SAPPORO_CODES`・`allSapporoSelected` などがそれにあたる．
Gemini レビューで「UI コンポーネントにドメインロジックを持たせるべきでない」という指摘を受けた．
`regionActions?: Record<string, RegionAction>` プロップに変更した．
...
空配列を渡すと `[].every(...)` が `true`（vacuous truth）を返す．
`setSelected` が呼ばれて不要な再レンダリングを引き起こす問題だった．
...
`allSapporoSelected: boolean` を `CheckboxList` のプロップとして渡す案を検討した．
コンポーネント内でラベル切り替えを行う設計だった．
...
ボタンはラベルと `onClick` の有無でのみ制御されるため，防御コードが不要になった．
...
コンポーネントがドメイン名（`石狩振興局`）や定数（`SAPPORO_CODES`）を直接参照すると，汎用 UI 部品ではなくなる．
そのコンポーネントは「ドメイン固有部品」になってしまう．
