...
`municipalities.json` のキーは 5 桁コードで，将来の GeoJSON 照合に必要なため，
...
`NaN`（float 型）が来た場合，`len()` で `TypeError` が発生する実績がある．
...
`shell=True` で `subprocess.run` を呼ぶと，コマンドが存在しなくても `FileNotFoundError` は発生しない．
シェルがエラーメッセージを出力して非ゼロのリターンコードを返す．`except FileNotFoundError` が空振りになる．
...
`.prj` ファイルが欠落または破損した Shapefile を読み込むと，`gdf.crs` が `None` になる．
後続の `to_crs()` で `ValueError: Cannot transform naive geometries.` が発生する．

**対策**: `read_file()` 直後に `gdf.crs is None` を確認する．
N03 データの標準 CRS は EPSG:6668（JGD2011 地理座標系）で，
`set_crs("EPSG:6668")` でフォールバック設定する．

## レビューで学んだこと

### math.fsum による浮動小数点の累積誤差の抑制
...
簡略化の結果として自己交差（self-intersection）を生じさせることがある．
`make_valid()` を挟むことでフロントエンドや GIS ツールでの描画バグを防ぐ．

### aggfunc="first" を dissolve に明示する

`gdf.dissolve(by="code")` は将来の geopandas バージョンで `FutureWarning` を出す可能性がある．
非数値カラムの集計方法の変更が理由となる．
`aggfunc="first"` を明示することでバージョン間の挙動差を封じる．

### shutil.which で mapshaper の存在チェックを一元化

`shutil.which("mapshaper") or "mapshaper"` は `which` が `None` でもフォールバックを渡してしまう．
本来検出したかった「未インストール」の状態を隠蔽する欠陥がある．

`mapshaper_bin = shutil.which("mapshaper")` の後に None チェックを挟む．
...
属性名アクセスによる可読性向上の効果は認める．
しかし 188 件のデータ行すべてに `Municipality(...)` コンストラクタを追加する必要があり，
変更量に対する実益が小さいと判断して見送った．
データ行の大規模変更時に合わせて導入する．
...
- 総務省・全国の地方公共団体コード（2024 年 6 月 26 日現在）: https://www.eltax.lta.go.jp/documents/10705
