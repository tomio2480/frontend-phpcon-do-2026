...
本ノートは，Dependabot が起票した 10 件の依存更新 PR を，
サプライチェーン攻撃を念頭に調査して取り込んだ際の知見をまとめる．
対象は，SHA ピン運用での Actions 更新の検証手順，Dependabot PR で
CI が走らない構成上の落とし穴，版コメントの乖離，連続マージ時の
ロックファイル競合である．
...
結果として Dependabot PR では実テストもビルドも走らず，CI が実質
ノーガードであった．「CI 通過確認」を自動結果に頼れない．

### 対応

npm 5 件（jsdom 26→29 の major を含む）を 1 ブランチへまとめて反映し，
ローカルで `pnpm test`（82 passed）と `pnpm build` の成功を確認してから
マージした．Actions の major は GitHub 上でしか実行できないため，
SHA 検証と公式ソースをもって判断し，マージ後の `push: main` による
deploy 成功を実地検証とした．
...
Actions の major 更新後，ワークフロー内の併記コメントが旧版表記の
まま残っていた（例: SHA は v6.4.0 なのにコメントは `# v4`）．

### 判断

Dependabot は SHA は書き換えるが，人間向けの版コメントは更新しない．
major 取り込みを重ねるとコメントと実バージョンが乖離し，監査時の
誤認を招く．別行コメントだけでなく，パス指定 Action の inline コメントも
更新されない事例を確認した．

### 対応

マージ後に別 PR でコメントを実 SHA に対応する版へ同期した．対象は
`deploy.yml` の checkout・setup-node・configure-pages・upload-pages-artifact，
および `md-lint.yml` の checkout・github-workflows の計 6 箇所である．
...
- マージした PR: #47, #52, #53, #54, #55, #56, #57, #58, #59, #60
- コメント同期 PR: #62
