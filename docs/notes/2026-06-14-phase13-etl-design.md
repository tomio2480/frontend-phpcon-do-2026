...
`scripts/build_municipalities.py` に面積・人口・ふるさと納税データがハードコードされており，
データ更新のたびにソースコードを直接編集する必要があった．
...
| `etl_furusato.py` | `scripts/data/furusato.csv` | 年1回（総務省現況調査） |

`build_municipalities.py` の `MUNICIPALITIES` リストは `(code, name, display_name, region)` の
4 列のみに縮小した．面積・人口・ふるさと納税は ETL 経由で結合する．

### 面積値に公式統計ではなく GeoJSON 計算値を採用した理由

- 公式値（国土地理院「全国都道府県市区町村別面積調」）は別途 CSV 管理が必要になる
...
`furusato.csv` では `01100`（札幌市全体）を 1 行として持ち，
...
既知の制約として `build_municipalities.py` の `SAPPORO_FURUSATO_AMOUNT` /
`SAPPORO_FURUSATO_COUNT` 定数と CSV が二重管理になっている．
将来は CSV を単一ソースとし定数を削除することを検討する．
...
総務省「全国地方公共団体コード」（2024年6月26日現在）で確認済み．
